...
function parsePrice(priceStr) {
  ...
  const cleaned = priceStr.replace(/[,$Ã‚Â£Ã¢â€šÂ¬Ã‚Â¥Ã¢â€šÂ¹]/g, '').replace(/\s+/g, ' ').trim();
  const patterns = [
    /(\d+\.?\d*)/,
    /(\d+,\d+\.?\d*)/,
    /(\d+\.\d{2})/,
...
function optimizeImageUrl(imageUrl) {
  ...
  try {
    ...
    console.log('Could not optimize image URL:', error);
    return imageUrl;
  }
}
...
function getBrandIcon(store, productUrl = null) {
  ...
  if (productUrl) {
    try {
      ...
      console.log('Could not parse product URL:', productUrl);
    }
  }
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    chrome.tabs.sendMessage(activeTab.id, { action: 'checkButtonState' }, (response) => {
      if (chrome.runtime.lastError) {
        console.log('Could not check button state:', chrome.runtime.lastError);
...
function toggleSaveButton() {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    chrome.scripting.executeScript({
      ...
      if (chrome.runtime.lastError) {
        console.log('Script injection result:', chrome.runtime.lastError);
      ...
      chrome.tabs.sendMessage(activeTab.id, { action: 'toggleSaveButton' }, (response) => {
        if (chrome.runtime.lastError) {
          console.log('Message sending result:', chrome.runtime.lastError);
        }
...
function togglePriceHistory(productIndex) {
  ...
  if (!historyDiv || !toggleBtn) {
    console.error('Could not find history elements for product:', productIndex);
    return;
  }
  
  if (historyDiv.style.display === 'none' || !historyDiv.style.display) {
    historyDiv.style.display = 'block';
    toggleBtn.textContent = 'ðŸ“ˆ Hide History';
    toggleBtn.classList.add('active');
  } else {
    historyDiv.style.display = 'none';
    toggleBtn.textContent = 'ðŸ“Š Price History';
...
function renderPagination() {
  ...
  controlsHTML += `<button id="prev-btn" class="pagination-arrow" ${currentPage === 1 ? 'disabled' : ''} title="Previous page">â´</button>`;
  ...
  controlsHTML += `<button id="next-btn" class="pagination-arrow" ${currentPage >= totalPages ? 'disabled' : ''} title="Next page">âµ</button>`;
  ...
  console.log('Pagination HTML created');
...
function renderProducts() {
  ...
  if (totalItems === 0) {
    ...
    if (showingFavorites) {
      ...
      emptyMessage = `
        <div class="empty-state">
          <p>ðŸªž Your dressing room is empty</p>
  ...
  pageItems.forEach((item) => {
    ...
    const favoriteIcon = item.isFavorite ? 'â­' : 'â­ï¸Ž';

    // Check if current item is a sale - safely handle missing sale dataâ˜†â˜†â˜†
    ...
    console.log('ðŸªž POPUP: Rendering product:', item.name);
    console.log('ðŸªž POPUP: Price history:', currentPriceHistory);
    console.log('ðŸªž POPUP: Is sale:', isSale);
    ...
    productDiv.innerHTML = `
      ...
      <div class="info">
        ...
        ${priceDisplay}
        <p class="item-meta">
          ${item.store} â€¢ ${new Date(item.savedAt).toLocaleDateString()}
          ...
          ${isSale ? ' <span class="sale-indicator">ðŸ·ï¸ SALE</span>' : ''}
      ...
      <button data-index="${globalIndex}" class="delete-btn" aria-label="Delete ${item.name}" title="Delete item">ðŸ—‘ï¸</button>
    `;
...
async function renderBrandFilters(searchTerm = brandSearchTerm) {
  console.log('Ã°Å¸â€Â renderBrandFilters called with:', searchTerm);
  console.log('Ã°Å¸â€Â Currently focused element:', document.activeElement);
  ...
  searchInput.placeholder = 'ðŸ” Search brands...';
  ...
  searchInput.addEventListener('input', (e) => {
    console.log('Ã°Å¸â€Â Input event fired:', e.target.value);
    console.log('Ã°Å¸â€Â isTyping:', isTyping);
    
    e.stopPropagation();
    brandSearchTerm = e.target.value.trim();
    
    if (isTyping) {
      console.log('Ã°Å¸â€Â Already typing, skipping render');
      return;
    }
    ...
    setTimeout(() => {
      console.log('Ã°Å¸â€Â Executing delayed render');
  ...
  searchInput.addEventListener('keydown', (e) => {
    console.log('Ã°Å¸â€Â Keydown event:', e.key);
    e.stopPropagation();
  });
  
  searchInput.addEventListener('keyup', (e) => {
    console.log('Ã°Å¸â€Â Keyup event:', e.key);
    e.stopPropagation();
  });
  
  searchInput.addEventListener('focus', (e) => {
    console.log('Ã°Å¸â€Â Focus event');
    e.stopPropagation();
  });
  
  searchInput.addEventListener('blur', (e) => {
    console.log('Ã°Å¸â€Â Blur event');
    e.stopPropagation();
  });
  
  searchInput.addEventListener('click', (e) => {
    console.log('Ã°Å¸â€Â Click event on search input');
    e.stopPropagation();
  });
  ...
  favoritesBtn.addEventListener('click', (e) => {
    console.log('Ã°Å¸â€Â Favorites button clicked');
    e.stopPropagation();
  ...
  if (uniqueStores.length === 0) {
    ...
    console.log('No brands, focusing search input');
  ...
  displayBrands.forEach(store => {
    ...
    img.addEventListener('click', (e) => {
      console.log('Brand icon clicked:', store);
      e.stopPropagation();
  ...
  if (!searchTerm) {
    ...
    if (totalBrands > 5) {
      ...
      toggleButton.addEventListener('click', (e) => {
        console.log('Show all/less button clicked');
        e.stopPropagation();
  ...
  console.log('About to focus search input');
  // Always try to focus and restore cursor position
  setTimeout(() => {
    ...
    if (newSearchInput) {
      ...
      console.log('Search input focused, cursor at end');
    }
  }, 0);
}
...
function getPriceFreshness(lastChecked) {
  if (!lastChecked) return { class: 'freshness-stale', text: 'Never updated', icon: 'ðŸ”´' };

  const hours = (Date.now() - new Date(lastChecked)) / (1000 * 60 * 60);

  if (hours < 6) return { class: 'freshness-fresh', text: 'Fresh', icon: 'ðŸŸ¢' };
  if (hours < 24) return { class: 'freshness-recent', text: 'Recent', icon: 'ðŸŸ¡' };
  return { class: 'freshness-stale', text: 'Stale', icon: 'ðŸŸ ' };
}

function openCompareWindow() {
  // Open compare.html in a new window
...
async function checkSinglePrice(product) {
  ...
  try {
    ...
    console.error(`âŒ Error checking ${product.name}:`, error);
    return {
      success: false,
      error: error.message
    };
...
function extractPriceSemantically(doc) {
  ...
  for (const el of priceElements) {
    ...
    if (match && text.length < 50) {
      ...
      if (isOldPrice) {
        console.log(`   â­ï¸ Skipping strikethrough price: ${match[0]}`);
        continue;
      }
  ...
  if (candidates.length > 0) {
    ...
    console.log(`   âœ… Found price: ${best.price} (score: ${best.score}, isSale: ${best.isSale})`);
...
async function checkAllPrices() {
  ...
  checkButton.innerHTML = 'â³ Checking prices...';
  ...
  currentItemText.innerHTML = `<strong>âœ… Complete!</strong> Checked ${products.length} items`;
  ...
  if (priceDrops > 0) {
    checkButton.innerHTML = `ðŸŽ‰ ${priceDrops} price drop${priceDrops !== 1 ? 's' : ''} found!`;
    checkButton.style.background = 'linear-gradient(135deg, #7a9b76, #96CEB4)';
  } else if (priceIncreases > 0) {
    checkButton.innerHTML = `âš ï¸ ${priceIncreases} price increase${priceIncreases !== 1 ? 's' : ''}`;
    checkButton.style.background = 'linear-gradient(135deg, #c65d47, #d4a574)';
  } else {
    checkButton.innerHTML = 'âœ… All prices current';
  }
  ...
  setTimeout(() => {
    ...
    checkButton.innerHTML = 'ðŸ”„ Refresh Prices';
